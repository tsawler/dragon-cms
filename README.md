...
class PageManager {
    ...
    async savePage() {
        ...
        const content = this.editor.exportHTML();
        
        await fetch(`/api/pages/${this.currentPage}/content`, {
            method: 'PUT',
    ...
    addPageToSidebar(page) {
        ...
        pageItem.addEventListener('click', () => this.loadPage(page.id));
        sidebar.appendChild(pageItem);
    }
    
    setupEventListeners() {
        // Save on mode change to display
        window.addEventListener('dragonModeChanged', (e) => {
            if (e.detail.mode === 'display') {
                this.savePage();
            }
